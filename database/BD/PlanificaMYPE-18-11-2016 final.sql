...
CREATE TABLE IF NOT EXISTS `BDPlanificaMYPE`.`DetalleViaje` (
  ...
  PRIMARY KEY (`idViaje`, `idArticulo`, `idPedido`),
  ...
  CONSTRAINT `FK_DetalleViaje_viaje`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_DetalleViaje_Pedido`
    FOREIGN KEY (`idPedido`)
    REFERENCES `BDPlanificaMYPE`.`Pedido` (`idPedido`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `FK_DetalleViaje_Pedido_idx` ON `BDPlanificaMYPE`.`DetalleViaje` (`idPedido` ASC);
